...
dali_nf %>%
  ...
  distinct() %>%
  mutate(genome_type = hash_lookup(genome_types_hash, target_family))
...
dali_nf %>%
  filter(query == "Dioxygenase") %>%
  filter(z >= 8) %>%
  filter(target_family %in% c("Betaflexiviridae", "Potyviridae", "Closteroviridae", "Alphaflexiviridae")) %>%
  filter(!grepl("PART", target))
